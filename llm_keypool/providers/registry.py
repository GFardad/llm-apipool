...
Usage::
    ...
    from llm_keypool.providers.registry import register, get_provider, list_providers
...
def _ensure_discovered() -> None:
    ...
    _import_module("llm_keypool.providers.cloudflare")
    _import_module("llm_keypool.providers.cohere")
    _import_module("llm_keypool.providers.openai_compat")
...
def _discover_adapters() -> None:
    ...
    try:
        pkg = importlib.import_module("llm_keypool.providers.adapters")
        for _finder, name, _ispkg in pkgutil.iter_modules(pkg.__path__):
            _import_module(f"llm_keypool.providers.adapters.{name}")
