...
def _build_key_data(provider: str, key: str, configs: dict[str, Any] | None = None) -> dict[str, Any]:
    data = configs or load_provider_configs()
    cfg = data[provider]
...
async def _call_provider(provider: str, key: str, timeout: float, configs: dict[str, Any]) -> CompletionResult:
    key_data = _build_key_data(provider, key, configs)

    if provider == "cohere":
        from llm_keypool.providers.cohere import complete

        async with asyncio.timeout(timeout):
            result = await complete(key_data, _TEST_MESSAGES, stream=False)
        if not isinstance(result, CompletionResult):
            return CompletionResult(text="", tokens_used=0, was_429=False, error="provider returned a stream")
        return result

    if provider == "cloudflare":
        from llm_keypool.providers.cloudflare import complete
    ...
    from llm_keypool.providers.openai_compat import complete
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m llm_keypool.key_checker <api_key> [provider1,provider2,...]")
        raise SystemExit(1)
    ...
    print(f"Testing key: {api_key[:8]}****{api_key[-4:] if len(api_key) > 12 else api_key}")
