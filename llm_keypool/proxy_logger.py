...
_LOG_DIR = Path.home() / ".llm-keypool" / "logs"
...
def write_entry(
    ...
    *,
    ...
    entry: dict[str, Any] = {
        ...
        "key_id": key_id,
        "stream": stream,
        "request_body": _truncate(request_body, _MAX_BODY_LOG) if request_body else None,
        "response_body": _truncate(response_body, _MAX_BODY_LOG) if response_body else None,
    }
...
def get_stats(days: int = 1) -> dict[str, Any]:
    ...
    success = sum(1 for e in entries if e.get("status_code", 0) < 400 or not e.get("error"))
    ...
    for e in entries:
        ...
        if e.get("status_code") and e["status_code"] >= 400:
            errors_by_code[e["status_code"]] = errors_by_code.get(e["status_code"], 0) + 1
