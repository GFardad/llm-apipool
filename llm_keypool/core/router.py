...
def get_all_penalties() -> list[dict[str, Any]]:
    ...
    for model_db_id, entry in list(_rate_limit_penalties.items()):
        ...
        if penalty > 0:
            result.append({"model_db_id": model_db_id, "count": entry["count"], "penalty": penalty})
...
def set_custom_weights(weights: CustomWeights) -> None:
    ...
    if not all(isinstance(v, (int, float)) and v >= 0 for v in (weights.reliability, weights.speed, weights.intelligence)):
...
def weights_for(strategy: str) -> Optional[CustomWeights]:
    ...
    if preset:
        return CustomWeights(reliability=preset.reliability, speed=preset.speed, intelligence=preset.intelligence)
    return CustomWeights(reliability=0.5, speed=0.25, intelligence=0.25)
