...
class FallbackManager:
    ...
    def _apply_cooldown(self, key_id: int, platform: str, model: str) -> None:
        ...
        self.store.record_usage(key_id, tokens=0, was_429=True, cooldown_until=expires_at)
    ...
    def _get_ordered_candidates(
        ...
        chain = self.store.get_enabled_models() if hasattr(self.store, "get_enabled_models") else []
        for entry in chain:
            platform = entry.get("platform", "")
            model_id = entry.get("model_id", "")
            model_db_id = entry.get("model_db_id") or entry.get("id", 0)
            keys = self.store.get_active_keys(
    ...
    def _should_skip_provider(self, platform: str) -> bool:
        return self._provider_attempts.get(platform, 0) >= self.MAX_ATTEMPTS_SAME_PROVIDER
    ...
    async def route_with_fallback(self, request: dict[str, Any]) -> Any:
        ...
        for model_group in candidates:
            ...
            for key in keys:
                if self._should_skip_key(key["id"]):
                    continue
                result = await self._try_key(key, model_id, platform, messages, subscriber_id)
        ...
        raise AllModelsExhaustedError(f"All models exhausted after {len(candidates)} candidates")
    ...
    async def _try_key(
        ...
        from ..providers.dispatch import complete
        ...
        try:
            ...
            self._provider_attempts[platform] = self._provider_attempts.get(platform, 0) + 1
