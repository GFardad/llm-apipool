...
async def check_key_health(store: KeyStore, key_id: int) -> str:
    ...
    if count >= CONSECUTIVE_FAILURES_TO_DISABLE:
        store.update_key_status(key_id, "invalid", enabled=False, last_checked_at=now_iso)
        _failure_counts.pop(key_id, None)
    else:
...
# Need update_key_status on KeyStore — check if it exists
if not hasattr(KeyStore, "update_key_status"):
    ...
    KeyStore.update_key_status = _update_key_status
