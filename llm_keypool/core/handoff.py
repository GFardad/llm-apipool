...
def get_handoff_mode() -> str:
    """Return ``'on_model_switch'`` or ``'off'`` based on the env var."""
...
def maybe_inject(
    ...
    already = any(
        m.get("role") == "system"
        and (
            (isinstance(m.get("content"), str) and m["content"].startswith("FreeLLMAPI context handoff:"))
            or (
                isinstance(m.get("content"), list)
                and any(
                    isinstance(p, dict) and p.get("text", "").startswith("FreeLLMAPI context handoff:")
                    for p in m["content"]
                )
            )
        )
