...
_SOURCE_PROVIDERS: list[dict[str, Any]] = []
"""Cached list of provider configs from providers.json."""


def _load_providers_config() -> list[dict[str, Any]]:
    """Return the list of provider dicts from ``providers.json``.
    ...
    try:
        ...
        _SOURCE_PROVIDERS = data if isinstance(data, list) else data.get("providers", [])
...
def refresh_from_db(store: Any = None, db_path: str | None = None) -> int:
    ...
    *db_path* string for a direct SQLite connection.  Returns the number
    ...
    if db_path is None:
        new_db = Path.home() / ".llm-keypool" / "keys.db"
        old_db = Path.home() / ".llm-aggregator" / "keys.db"
        db_path = str(new_db if new_db.exists() else old_db) if old_db.exists() else str(new_db)
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        for prov in providers:
            ...
            try:
                ...
                data = resp.json()
                results[prov.get("name", prov.get("id", "unknown"))] = data.get("data", [])
            except Exception:
                pass
