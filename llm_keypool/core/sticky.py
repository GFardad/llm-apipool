...
STICKY_TTL_MS = _sticky_settings.sticky_ttl_ms
MAX_STICKY_ENTRIES = _sticky_settings.max_sticky_entries
...
def _prune() -> None:
    ...
    for key, val in list(_store.items()):
        if now - val["last_used_ms"] > STICKY_TTL_MS:
            del _store[key]
    if len(_store) > MAX_STICKY_ENTRIES:
        sorted_items = sorted(_store.items(), key=lambda x: x[1]["last_used_ms"])
        for key, _ in sorted_items[: len(_store) - MAX_STICKY_ENTRIES]:
            _store.pop(key, None)
