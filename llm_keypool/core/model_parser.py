...
class ModelParser:
    ...
    _STRATEGY_PATTERN = re.compile(r"\.(priority|balanced|smartest|fastest|reliable|custom|auto)$")
    ...
    def parse(cls, model_string: str) -> tuple[str, ModelFilter, str | None]:
        ...
        if context_match:
            ...
            context_tokens = int(number) * (1000 if (suffix or "").lower() == "k" else 1000000)
            if operator in (">=", ">", "=>"):
